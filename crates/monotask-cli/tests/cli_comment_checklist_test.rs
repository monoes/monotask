...
/// Helper: create space+board+column+card, return (board_id, col_id, card_id)
fn setup_card(tmp: &std::path::Path) -> (String, String, String) {
    let sid = make_space(tmp);
    let board_out = cli(tmp, &["board", "create", "TestBoard", "--space", &sid, "--json"]);
...
fn comment_add_list_delete() {
    ...
    // Add comment
    let add_out = cli(tmp.path(), &["card", "comment", "add", &board_id, &card_id, "Hello world", "--json"]);
    ...
    // List comments
    let list_out = cli(tmp.path(), &["card", "comment", "list", &board_id, &card_id, "--json"]);
    ...
    // Delete comment
    let del_out = cli(tmp.path(), &["card", "comment", "delete", &board_id, &card_id, &comment_id, "--json"]);
    assert!(del_out.status.success());

    // List again — should be empty
    let list2_out = cli(tmp.path(), &["card", "comment", "list", &board_id, &card_id, "--json"]);
...
fn checklist_add_item_check() {
    ...
    // Add checklist
    let cl_out = cli(tmp.path(), &["checklist", "add", &board_id, &card_id, "QA Steps", "--json"]);
    ...
    // Add item
    let item_out = cli(tmp.path(), &["checklist", "item-add", &board_id, &card_id, &cl_id, "Write tests", "--json"]);
    ...
    // Check the item
    let check_out = cli(tmp.path(), &["checklist", "item-check", &board_id, &card_id, &cl_id, &item_id, "--json"]);
